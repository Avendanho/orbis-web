...
export async function resolveArticle(input:string,refresh=false):Promise<Article>{
 ...
 const results=await Promise.all([
  ...
  source('Semantic Scholar','https://api.semanticscholar.org/graph/v1/paper/DOI:'+encodeURIComponent(doi)+'?fields=title,authors,year,venue,openAccessPdf,isOpenAccess'),
 ...
 if(!cr){
  ...
  const d=dc.data?.data?.attributes;
 ...
 const reason=accessReason({pdf:!!pdf,oa:knownOa,paid,closed:oa?.open_access?.oa_status==='closed',found:!!(cr||ep||ss||oa),issues});
 const article:Article={doi,pageDoi:pageDoiVisto,abstract:(cr?.abstract||ep?.abstractText||'').replace(/<[^>]*>/g,''),title:cr?.title?.[0]?.replace(/<[^>]*>/g,'')||ep?.title||ss?.title||oa?.display_name||doi,authors:cr?.author?.slice(0,6).map((a:Row)=>[a.given,a.family].filter(Boolean).join(' ')).join(', ')||ep?.authorString||ss?.authors?.slice(0,6).map((a:Row)=>a.name).join(', ')||oa?.authorships?.slice(0,6).map((a:Row)=>a.author?.display_name).filter(Boolean).join(', ')||'',year:String(cr?.published?.['date-parts']?.[0]?.[0]||ep?.pubYear||ss?.year||oa?.publication_year||''),journal:cr?.['container-title']?.[0]||ep?.journalInfo?.journal?.title||ss?.venue||oa?.primary_location?.source?.display_name||'',pdf,pdfUrls:candidates.slice(0,12).map(c=>c.url),source:candidates[0]?.source||'',found:!!(cr||ep||ss||oa),partial:issues.length>0,freeUrl:freePages[0]||(knownOa?pages[0]?.url:undefined),sourceIssues:issues,...reason};
